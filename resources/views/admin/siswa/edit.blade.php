...
<head>
    ...
    <title>Edit Siswa - Admin SDN Kroyolor</title>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('peserta-didik.update', $pesertaDidik->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nama" class="form-control" value="{{ $pesertaDidik->nama }}" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Kelas</label>
                            <select name="kelas" class="form-select" required>
                                <option value="I" {{ $pesertaDidik->kelas == 'I' ? 'selected' : '' }}>I (Satu)</option>
                                <option value="II" {{ $pesertaDidik->kelas == 'II' ? 'selected' : '' }}>II (Dua)</option>
                                <option value="III" {{ $pesertaDidik->kelas == 'III' ? 'selected' : '' }}>III (Tiga)</option>
                                <option value="IV" {{ $pesertaDidik->kelas == 'IV' ? 'selected' : '' }}>IV (Empat)</option>
                                <option value="V" {{ $pesertaDidik->kelas == 'V' ? 'selected' : '' }}>V (Lima)</option>
                                <option value="VI" {{ $pesertaDidik->kelas == 'VI' ? 'selected' : '' }}>VI (Enam)</option>
                            </select>
                        </div>
                        <div class="mb-3">
